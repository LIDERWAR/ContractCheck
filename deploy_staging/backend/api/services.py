...
def extract_text_from_pdf(file_stream, max_pages=None):
    ...
    Возвращает: (text, pages_processed, total_pages)
    """
    try:
...
def extract_text_from_docx(file_stream, max_pages=None):
    """
    Извлекает текст из .docx файла. Условно считает 1800 символов за страницу.
    Возвращает: (text, pages_processed, total_pages)
    """
    try:
...
def extract_text_from_txt(file_stream, max_pages=None):
    """
    Извлекает текст из .txt файла. Оценка страниц по символам (~1800 = 1 стр).
    Возвращает: (text, pages_processed, total_pages)
    """
    try:
...
def analyze_contract_with_ai(contract_text, force_mock=False):
    ...
    try:
        ...
        for edit in edits:
            ...
            if original and replacement and original in rewritten_text:
