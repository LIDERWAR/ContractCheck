document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function loadUserInfo() {
        try {
            ...
            if (response.ok) {
                ...
                // Update Name
                let displayName = user.username || user.email || user.first_name || 'Пользователь';

                // If displayName is email, let's keep it or show prefix
                ...
                const tier = (user.profile && user.profile.subscription_tier) ? user.profile.subscription_tier : 'free';
                ...
                const checksRemaining = (user.profile && user.profile.checks_remaining !== undefined) ? user.profile.checks_remaining : 0;
                const checksEl = document.querySelector('.checks-remaining-display');
                if (checksEl) {
                    checksEl.textContent = checksRemaining;
                }

                // Update Subscription badge in card
                const badgeEl = document.querySelector('.subscription-badge-display');
                if (badgeEl) {
                    badgeEl.textContent = 'Тариф ' + tier.charAt(0).toUpperCase() + tier.slice(1);
                }
    ...
    // Логика выхода теперь централизованно обрабатывается в main.js через делегирование событий
    ...
    async function deleteDocument(id) {
        window.showConfirm('Удалить документ?', 'Вы уверены, что хотите безвозвратно удалить этот файл?', async () => {
            const token = localStorage.getItem('cc_token');
            if (!token) {
                window.showAlert('Ошибка', 'Токен не найден. Пожалуйста, войдите снова.');
                return;
            }
